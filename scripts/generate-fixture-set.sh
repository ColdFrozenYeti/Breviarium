...
#
# Beta 2's day hours take the hour as a fourth argument (DO's name: Laudes, Prima, Tertia,
# Sexta, Nona, Completorium):
#   hours          Vulgate Latin + English, priest off, "rows" -> hours/<hour>/<year>.tar.gz
#   hours-bea      Pius XII Latin only, priest off, "flat"     -> hours-bea/<hour>/<year>.tar.gz
#   hours-holdout  Vulgate Latin + English, priest off and on  -> holdout/<year>-<hour>.tar.gz
#
...
case "$set_name" in
  ...
  hours)     lang2=English; format=rows; suffix=bilingual.tsv; priests="0"; dest="hours/$hour" ;;
  hours-bea) lang1=Latin-Bea; lang2=Latin-Bea; format=flat; suffix=latin.txt; priests="0"; dest="hours-bea/$hour" ;;
